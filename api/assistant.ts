export default async function handler(req: any, res: any) {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${process.env.GROQ_API_KEY}` }, body: JSON.stringify({ model: 'openai/gpt-oss-20b', messages: [{ role: 'system', content: 'Sen Klas Sosyal uygulamasının Klas AI adlı kişisel asistanısın. Türkçe, kısa, yararlı ve güvenli cevaplar ver. Önceki konuşma bağlamını kullan.' }, ...history, { role: 'user', content: message }], temperature: 0.7, max_tokens: 500 }) });
