...
export default function GroupsScreen({ onClose }: Props) {
  ...
  async function loadGroups() {
    ...
    if (result.error?.message.includes('schema cache')) {
      ...
      const response = await fetch(`${supabaseProjectUrl}/rest/v1/groups?select=*&order=created_at.desc`, { headers: { apikey: import.meta.env.VITE_SUPABASE_ANON_KEY || import.meta.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY || import.meta.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY, ...(sessionData.session?.access_token ? { Authorization: `Bearer ${sessionData.session.access_token}` } : {}) } })
      const fallbackData = response.ok ? await response.json() as Group[] : []
      result = { data: fallbackData, error: response.ok ? null : result.error, count: null, status: response.status, statusText: response.statusText } as unknown as typeof result
    }
    if (result.error) {
  ...
  return <main className="mx-auto min-h-full max-w-3xl p-4" aria-label="Gruplar">
    ...
    {loading ? <p className="py-10 text-center text-sm text-slate-500">Gruplar yükleniyor...</p> : <div className="grid gap-3 sm:grid-cols-2">{groups.map((group) => <article key={group.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="mb-3 flex items-start justify-between"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-100 text-slate-500"><Users className="h-5 w-5" /></div><span className="rounded-full bg-emerald-50 px-2 py-1 text-[11px] font-semibold text-emerald-700">Açık grup</span></div><h2 className="font-bold text-slate-900">{group.name}</h2><p className="mt-1 line-clamp-2 text-sm text-slate-500">{group.description || 'Bu grubun henüz bir açıklaması yok.'}</p><button type="button" onClick={() => void joinGroup(group)} className="mt-4 w-full rounded-xl border border-violet-200 px-3 py-2 text-sm font-semibold text-violet-700">Gruba katıl</button></article>)}</div>}
