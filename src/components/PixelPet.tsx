...
type Pet = { name: string; xp: number; level: number; hunger: number; last_interaction_at: string; color: PetColor; shape: PetShape; outfit: string; habitat: string; coins: number };
...
const defaultPet: Pet = { name: 'Piksel', xp: 0, level: 1, hunger: 100, last_interaction_at: new Date().toISOString(), color: 'orange', shape: 'round', outfit: 'none', habitat: 'cardboard-box', coins: 100 };
...
export default function PixelPet() {
  ...
  useEffect(() => {
    if (!user) return;
    let active = true;
    supabase.from('pixel_pets').select('name,xp,level,hunger,last_interaction_at,color,shape,outfit,habitat,coins').eq('user_id', user.id).maybeSingle().then(async ({ data }) => {
      let next = data as Pet | null;
      if (!next) {
        const { data: created } = await supabase.from('pixel_pets').insert({ user_id: user.id }).select('name,xp,level,hunger,last_interaction_at,color,shape,outfit,habitat,coins').single();
  ...
  const buyOrEquip = async (item: ShopItem) => {
    ...
    const patch = item.category === 'outfit' ? { outfit: item.id, coins: pet.coins - item.price } : { habitat: item.id, coins: pet.coins - item.price };
    await savePet(patch);
  };
  ...
  return (
    <section className="mb-5 overflow-hidden rounded-2xl border-2 border-orange-200 bg-[#fffaf3] p-4 text-slate-900 shadow-[4px_4px_0_#fed7aa]" aria-label="Piksel kedi evcil hayvanı">
      ...
      {shopOpen && <div className="mt-3 rounded-xl border border-orange-200 bg-white p-3"><div className="flex items-center justify-between"><p className="flex items-center gap-2 text-sm font-black"><ShoppingBag className="h-4 w-4 text-orange-500" /> Kedi Mağazası</p><span className="text-xs font-bold text-orange-500">{pet.coins} para</span></div><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => setTab('outfit')} className={`rounded-lg px-2 py-2 text-xs font-bold ${tab === 'outfit' ? 'bg-orange-100 text-orange-700' : 'bg-slate-50'}`}>Giydir</button><button type="button" onClick={() => setTab('habitat')} className={`rounded-lg px-2 py-2 text-xs font-bold ${tab === 'habitat' ? 'bg-orange-100 text-orange-700' : 'bg-slate-50'}`}>Yuvalar</button></div><div className="mt-3 grid grid-cols-2 gap-2">{selectedItems.map((item) => <button key={item.id} type="button" onClick={() => buyOrEquip(item)} disabled={pet.coins < item.price} className="flex items-center justify-between rounded-lg border border-slate-100 p-2 text-left text-xs transition hover:border-orange-300 disabled:opacity-40"><span><span className="block font-bold">{item.name}</span><span className="text-slate-500">{item.price} para</span></span><span className="h-5 w-5 rounded-full" style={{ backgroundColor: item.accent }} /></button>)}</div></div>}
    </section>
  );
}
