...
export default function PersonalAssistant({ displayName = 'arkadaşım' }: Props) {
  ...
  return <section className="mx-auto w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" aria-label="Klas AI kişisel asistanı">
    <div className="mb-4 flex items-center gap-3"><div className="relative flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-100 shadow-sm" aria-label="Klas AI logosu"><span className="absolute bottom-2 left-2.5 h-6 w-4 -rotate-[28deg] rounded-b-full rounded-tl-full border-[4px] border-t-0 border-slate-600" /><span className="absolute bottom-2 right-2.5 h-6 w-4 rotate-[28deg] rounded-b-full border-[4px] border-t-0 border-slate-600" /><span className="absolute right-3 top-2 h-2 w-2 rounded-full bg-slate-600" /></div><div><div className="flex items-center gap-2"><h2 className="font-bold text-slate-900">Klas AI</h2><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold text-emerald-700">Çevrimiçi</span></div><p className="text-xs text-slate-500">Günlük işlerden karmaşık problemlere kadar yardımcı olur</p></div></div>
