    ...
    >
      <div className="max-w-xl mx-auto flex items-center justify-between gap-2 px-4 py-2.5">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {onAssistantClick && <button onClick={onAssistantClick} className="relative flex h-10 w-10 items-center justify-center rounded-full transition-all hover:scale-105" style={{ background: cardBg, border: `1px solid ${cardBorder}` }} aria-label="Kişisel asistan"><span className="relative flex h-5 w-5 items-center justify-center" aria-hidden="true"><span className="absolute bottom-0 left-0 h-3.5 w-2.5 -rotate-[28deg] rounded-b-full rounded-tl-full border-[3px] border-t-0" style={{ borderColor: subtextColor }} /><span className="absolute bottom-0 right-0 h-3.5 w-2.5 rotate-[28deg] rounded-b-full border-[3px] border-t-0" style={{ borderColor: subtextColor }} /><span className="absolute right-1 top-0 h-1.5 w-1.5 rounded-full" style={{ backgroundColor: subtextColor }} /></span></button>}
